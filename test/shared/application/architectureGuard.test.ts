...
/** The dashboard dir is scanned for BOTH extensions: a plain-`.ts` helper dropped in here
 *  (a chip map, a label table) is just as capable of hard-coding a category id as a `.tsx`
 *  component, and a `.tsx`-only walk would never look at it. */
...
const files = [
  ...
  ...SCAN_DIRS_TSX.flatMap((d) => walk(d, ['.ts', '.tsx'])),
];
